...
if len(sys.argv) != 5:
    sys.exit('\n Usage: basf2 SelectPhiK0_c3.py `action` `sample` `file_name_stub` `output file name`.\n\n Where `action` = `simple`, `training`, or `expert`\n and `sample` = `signal`, `BBbar` or `continuum`\n'
    )
...
vertexRave('B0:ch3', 0.0, 'B0:ch3 -> [phi -> pi0 ^pi+ ^pi-] ^K_S0', 'iptube')
matchMCTruth('B0:ch3')
...
if action == 'expert':
    ...
    for method in methods:
        ...
        expert.param('workingDirectory', 'training_ch3')
...
if action == 'expert':
    ...
    toolsBsigCh3 += ['CustomFloats[csv_BDT]', '^B0:ch3']
    toolsBsigCh3 += ['CustomFloats[csv_HMatrix]', '^B0:ch3']
    toolsBsigCh3 += ['CustomFloats[csv_Fisher]', '^B0:ch3']
    toolsBsigCh3 += ['CustomFloats[csv_LD]', '^B0:ch3']
    toolsBsigCh3 += ['CustomFloats[csv_Likelihood]', '^B0:ch3']
